...
def construct_and_place_cavern_and_labs(instr: core.InstrumentationData) -> None:
    ...
    if instr.detail["cavern"] != "omit":
        ...
        watertank_offset = (8877.6/2.)
        cavern_z_displacement =  cavern_extent[0][2] + watertank_offset + 1000 #world_extent[1][2] - cavern_extent[1][2]
        g4.PhysicalVolume(
...
def construct_cavern(material: g4.Material, registry: g4.Registry,  world_lv: g4.LogicalVolume) -> g4.LogicalVolume:
    ...
    if distance_center_to_end_of_tunnle > x_length/2.:
        distance_center_to_end_of_tunnle = x_length/2.
    ...
    cavern_box = g4.solid.Box("cavern_box",
    ...
    tube_cutout_h = box_cutout_x / 2.
    tube_cutout_r_1 = total_height - onset_of_curvature
    tube_cutout_r_2 = total_width / 2.
    ...
    box_z_offset = onset_of_curvature/2. + height_rock_below - z_length_smaller/2.                  #box_cutout_z - total_height) / 2. #-5000
    box_x_offset = (box_cutout_x - x_length) / 2.
    ...
    tube_z_offset =  box_z_offset - box_cutout_z/2. + total_height - tube_cutout_r_1 #box_z_offset + onset_of_curvature/2.
    cavern_box_wo_box_and_tube = g4.solid.Subtraction(
        "cavern_box_wo_box_and_tube",
        cavern_box_wo_box,
        cavern_cutout_tube,
        [[0, pi / 2.,0],[box_x_offset, 0, tube_z_offset]],
        registry
    )

    icarus_offset = box_z_offset - box_cutout_z/2. - tank_pit_height/2.
    ...
    cavern_lv = g4.LogicalVolume(cavern_box_wo_box_and_tube_and_icarus, material, "cavern", registry)
    return cavern_lv
