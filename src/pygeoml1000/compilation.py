"""Compilation of the pre-compiled configs into the objects the geometry is built from.

The pre-compiled configs describe the shape of the setup: how many strings, how
many detectors on a string, where the PMTs sit. This module turns them into the
two compiled objects, ``channelmap`` and ``special_metadata``, which name every
channel one by one.

:func:`pygeoml1000.config.resolve_config` runs this step. A config that already
holds both compiled objects skips it.
"""
...
from .utils import convert_to_plain_types, normalize_records
...
def generate_channelmap(
    ...
    crystal_records = normalize_records(configs.get("crystal"))
    hpge_records = normalize_records(configs.get("hpge"))
...
def compile_pre_compiled_config(configs: dict) -> tuple[dict, dict]:
    """Compile the pre-compiled configs into the objects the geometry is built from.

    Parameters
    ----------
    configs
        the pre-compiled configs, keyed by file name without its extension. Use
        :func:`pygeoml1000.config.load_pre_compiled_config` to obtain them.
